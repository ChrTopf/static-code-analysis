...
class RegionNewline(Check):
    ...
    def execute_on_changed_file(self, changed_file: LoadedFile) -> list[LineAnalysisIssue]:
        ...
        for changed_line in changed_file.changed_lines:
            self.__perform_check_for_line(changed_file, changed_line, issues)
        return issues
                
    def __perform_check_for_line(self, changed_file: LoadedFile, changed_line: ChangedLine, 
                                 issues: list[LineAnalysisIssue]):
        if self.__line_contains_region_tag(changed_line.content):
            self.__check_empty_line_before(changed_file, changed_line.number, issues)
            self.__check_empty_line_after(changed_file, changed_line.number, issues)
    ...
    def __check_empty_line_before(self, changed_file: LoadedFile, line_number: int, issues: list[LineAnalysisIssue]):
        line_before = line_number - 1
        if line_before < 0:
            return
        if not self.__is_line_empty(changed_file.all_lines[line_before]):
            issues.append(LineAnalysisIssue(line_before, f"Line {line_number} contains a region tag, but is "
                                                         f"missing an empty line before the region."))
            
    def __check_empty_line_after(self, changed_file: LoadedFile, line_number: int, issues: list[LineAnalysisIssue]):
        ...
        if not self.__is_line_empty(changed_file.all_lines[line_after]):
            issues.append(LineAnalysisIssue(line_after, f"Line {line_number} contains a region tag, but is "
