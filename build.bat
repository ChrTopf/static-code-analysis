...
:: build application
.venv\Scripts\pyinstaller.exe --onefile --noconsole --name "%APP_NAME%" --distpath ".\dist\V%TIMESTAMP%" .\main.py -y
if %errorlevel% NEQ 0 (
...
copy config.json dist\V%TIMESTAMP%

:: copy contents of dist to target directory
set TARGET_DIR=..\..\Tools\Static Code Analysis
robocopy dist "%TARGET_DIR%" /E /DCOPY:DAT /R:10 /W:3
